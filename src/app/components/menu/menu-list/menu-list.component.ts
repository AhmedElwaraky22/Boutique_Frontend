...
export class MenuListComponent implements OnInit {
  ...
  ngOnInit(): void {
    // فرضًا أن هناك شرط يعتمد على المستخدم
    const userRole = this.sharedService.getRole(); // افترض أن هذه الخدمة تعيد دور المستخدم

    // تخصيص القائمة بناءً على الشرط
    this.menu = this.filterMenu(this.menu, userRole);
  }

  filterMenu(menu: CoreMenu[], userRole: string): CoreMenu[] {
    return menu
      .filter((section) => {
        // مثال: إخفاء sections بناءً على الدور
        if (userRole === 'Admin') {
          return section.id !== 'user-section'; // إخفاء قسم معين
        } else if (userRole === 'user') {
          return section.id !== 'store-section'; // إخفاء قسم آخر
        }
        return true; // الافتراضي
      })
      .map((section) => {
        // تكرار العناصر داخل كل section
        if (section.children) {
          section.children = this.filterMenu(section.children, userRole);
        }
        return section;
      });
  }
